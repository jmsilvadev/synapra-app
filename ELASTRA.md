...
- Use the `elastra_context` MCP tool as the default discovery step before opening many local files.
- Query with the `project_id` and `namespace` declared in this file.
- Follow `navigate_to` refs like `path:line` and `anchor_ref` refs like `path#section` before asking for more code reading.
- If search returns weak or empty evidence, call `elastra_status` for the same `project_id` and `namespace` before assuming Elastra has no data.
- Effective visibility is scoped by the authenticated API key and organization, not only by `project_id` and `namespace`.

## Project Context

- Query: `elastra workspace`
- Coverage: 0 matches, 0 documents, 0 sources
- Status: No indexed context was retrieved for this query yet.
- Next actions:
  - verify knowledge sync for this project and namespace
  - check indexing status before assuming Elastra has no data
  - if status is clear and search is still empty, inspect local code and write back findings using elastra_sync

## Available MCP Tools

### elastra_context

Get structured context for a task or question (primary discovery tool):
- project_id: "232ea6ee-8f92-5a7f-af32-dc35d58e3177" (required)
- namespace: "b5e08a4f-04b2-5796-a199-d1a68ae15a41" (default: "workspace")
- query: "your task or question" (required)
...
- project_id: "232ea6ee-8f92-5a7f-af32-dc35d58e3177" (required)
- namespace: "b5e08a4f-04b2-5796-a199-d1a68ae15a41" (default: "workspace")
- files: [{"source_path": "path/to/file", "content": "file content"}]

### elastra_memory_write

Store engineering decisions, discoveries, bugs, or explanations:
- project_id: "232ea6ee-8f92-5a7f-af32-dc35d58e3177" (required)
- namespace: "b5e08a4f-04b2-5796-a199-d1a68ae15a41" (default: "workspace")
- type: "decision", "bug", "todo", "refactor", "explanation", or "pattern"
- title: "Short title"
- content: "Detailed content"

### elastra_rules

Get organization and workspace rules:
- project_id: "232ea6ee-8f92-5a7f-af32-dc35d58e3177" (required)
- namespace: "b5e08a4f-04b2-5796-a199-d1a68ae15a41" (default: "workspace")
- type: "organization" or "workspace" or "all"
